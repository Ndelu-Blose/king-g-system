...
import { Search, AlertTriangle, Boxes, Sofa } from 'lucide-react';
import { BackButton } from '@/components/BackButton';
import { motion } from 'framer-motion';

const lowStockThreshold = 10;

export default function InventoryLounge() {
  const { inventory } = useInventory();
  const [search, setSearch] = useState('');
  const location = useLocation();
  const isLounge = location.pathname === '/inventory/lounge';
  ...
  const loungeValue = filtered.reduce((sum, i) => sum + i.loungeQty * i.costPrice, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="font-display text-2xl font-bold text-foreground">Lounge stock</h1>
        <p className="text-sm text-muted-foreground">Stock levels and value at the Lounge location</p>
      </div>
      ...
      <div className="flex gap-1 bg-secondary rounded-lg p-1 w-fit">
        ...
        <Link
          to="/inventory/lounge"
          className={`px-3 py-1.5 rounded-md text-xs font-medium capitalize transition-all ${
            isLounge ? 'gold-gradient text-primary-foreground' : 'text-muted-foreground hover:text-foreground'
          }`}
        >
          Lounge
        </Link>
        <Link
          ...
          className={`px-3 py-1.5 rounded-md text-xs font-medium capitalize transition-all ${
            location.pathname === '/inventory/warehouse' ? 'gold-gradient text-primary-foreground' : 'text-muted-foreground hover:text-foreground'
          }`}
        >
          Warehouse
        </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass-card p-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-muted-foreground">Lounge SKUs</p>
        ...
        <div className="glass-card p-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-muted-foreground">Lounge value</p>
            <p className="text-xl font-bold text-foreground">R{loungeValue.toLocaleString()}</p>
          </div>
        </div>
        <div className="glass-card p-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-muted-foreground">Low stock (Lounge)</p>
